...
export default function DrugInteractionList(props) {
    ...
    const callBackEditAction = useCallback((drugInteractionId)=>{
        props.actions.editDrugIntraction(drugInteractionId);
    });

    const callBackDeleteAction = useCallback((drugInteractionId)=>{ 
        props.actions.deleteDrugIntraction(drugInteractionId);
    });
                        ...
                        props.data.data.map((drugInteraction, index)=>{
                            return (
                                <TableRow  key={drugInteraction.id} >
                                    ...
                                    <TableCell>  
                                        <Link to={`edit/${drugInteraction.id}`}>
                                            <IconButton 
                                            color="primary" 
                                            size="small" 
                                            onClick={()=>callBackEditAction(drugInteraction.id)}  
                                        ...
                                        color="error" 
                                        size="small" 
                                        onClick={()=>callBackDeleteAction(drugInteraction.id)}  
