...
export default function ActiveIngredientList(props) {
    ...
    const callBackEditAction = useCallback((event)=>{
        const item = props.data.filter((activeIngredient) => {
            return +activeIngredient.id === +event.currentTarget.value 
        })
    ...
    const callBackDeleteAction = useCallback((event)=>{
        ...
        axios.delete(`api/v1/drugs/${props.drugId}/activeingredients/${activeIngredientId}`)
        .then((response)=>{
                                ...
                                props.data.map((activeIngredient, index)=>{
                                    return (
                                        <TableRow  key={activeIngredient.pivot.id} >
                                            ...
                                            <TableCell>{ activeIngredient.concentration }</TableCell>
                                            <TableCell>{ activeIngredient.format }</TableCell>
                                            <TableCell>
                                                <IconButton 
                                                color="primary" 
                                                size="small" 
